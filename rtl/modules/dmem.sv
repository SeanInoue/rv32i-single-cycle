...
module dmem #(
  ...
  // declare ram
  logic [XLEN-1:0] ram[DEPTH];

  // Drop byte-select bits [1:0] to get word index
  localparam int WordAddrWidth = $clog2(DEPTH);  // # of word the ram holds
  logic [WordAddrWidth-1:0] word_addr;
  assign word_addr = addr[WordAddrWidth+1:2];
        ...
        MEM_BYTE: begin
          unique case (addr[1:0])
            ...
            read_data = mem_unsigned
                        ? {24'b0, ram[word_addr][7:0]}
                        : {{24{ram[word_addr][7]}},  ram[word_addr][7:0]};
            2'b01:
            read_data = mem_unsigned
                        ? {24'b0, ram[word_addr][15:8]}
                        : {{24{ram[word_addr][15]}}, ram[word_addr][15:8]};
            2'b10:
            read_data = mem_unsigned
                        ? {24'b0, ram[word_addr][23:16]}
                        : {{24{ram[word_addr][23]}}, ram[word_addr][23:16]};
            2'b11:
            read_data = mem_unsigned
                        ? {24'b0, ram[word_addr][31:24]}
                        : {{24{ram[word_addr][31]}}, ram[word_addr][31:24]};
          endcase
        end
        MEM_HALF: begin
          unique case (addr[1])
            1'b0:
            read_data = mem_unsigned
                        ? {16'b0, ram[word_addr][15:0]}
                        : {{16{ram[word_addr][15]}}, ram[word_addr][15:0]};
            1'b1:
            read_data = mem_unsigned
                        ? {16'b0, ram[word_addr][31:16]}
                        : {{16{ram[word_addr][31]}}, ram[word_addr][31:16]};
          endcase
        end
        MEM_WORD: begin
